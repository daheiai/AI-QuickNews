...
2、product（产品工具）：AI 产品发布、新工具介绍、GitHub 项目、软件更新、新功能、API
...
5、industry（行业资讯）：研究论文、安全报告、融资、政策、公司动态、合作、行业观点与预测

# 判断标准（重要性排序）
高优先级内容（必须收录）：
- 新工具/新产品介绍：有人介绍了一个工具是什么、能做什么，即使你不认识这个产品也要收录
- 实用教程和技巧：使用心得、工作流分享、配置方法、踩坑经验
- 社区现象：多人同时讨论/使用/尝试同一个产品或功能，说明这是热点，值得报道
- 数据变化：产品数据（下载量、用户数、收益）的显著变化，无论涨跌
- 用户真实反馈：对产品的吐槽、踩坑、惊喜体验，这些比官方宣传更有价值
...
特别注意：
- 不要只追"大新闻"和大数字，实用的小工具、个人分享同样重要
- 判断时不要依赖你是否认识具体产品名，看内容是否有信息量
- 多条相关推文可以合并为一条快讯，但不要因此漏掉其他独立的有价值内容

额外信息：JustinLin610是Qwen团队成员，Hx1u0是Kimi团队成员。

# 输出数量要求
- 根据输入信源的质量，输出 5-8 条快讯
- 如果有价值的内容较多，宁可多输出几条，不要漏掉重要信息
- 如果信源质量较低，可以少于5条，但要确保收录了所有有价值的内容
...
- 每条快讯的 content 必须控制在 80 字以内，只保留最核心的信息
""").strip()
